    ...
    try {
      ...
      const products = images.map((image, index) => {
        ...
        return {
          ...
          variants: [
            {
              templateVariantId: selectedProduct.variants[0], // Use first variant for simplicity
              imagePlaceholders: [
                {
                  name: selectedProduct.printAreas[0], // Use first print area
                  fileUrl: image.preview // This would need to be uploaded to a public URL in real implementation
                }
              ]
            }
          ]
      ...
      const { data, error } = await supabase.functions.invoke('gelato-bulk-create', {
        body: {
          templateId: selectedProduct.id,
          publish: true, // Could be configurable
          products
        }
      });
